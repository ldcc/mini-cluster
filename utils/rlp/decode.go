...
func makeMapDecoder(typ reflect.Type, tag tags) (decoder, error) {
	...
	vtypeinfo, err := cachedTypeInfo1(typ.Elem(), tags{})
	if err != nil {
		return nil, err
	}
	var dec decoder
	dec = func(s *Stream, val reflect.Value) error {
		...
		if err := decodeMapElements(s, val, ktypeinfo.decoder, vtypeinfo.decoder); err != nil {
			return err
		}
		return wrapStreamError(s.ListEnd(), val.Type())
	}
...
func decodeMapElements(s *Stream, val reflect.Value, kdec, vdec decoder) error {
	val.Set(reflect.MakeMap(val.Type()))
	k := reflect.New(val.Type().Key()).Elem()
	v := reflect.New(val.Type().Elem()).Elem()
	i := 0
	for ; ; i++ {
		if err := kdec(s, k); err == EOL {
			break
		} else if err != nil {
			return addErrorContext(err, fmt.Sprint("[", i, "]"))
		}
		if err := vdec(s, v); err == EOL {
			break
		} else if err != nil {
			return addErrorContext(err, fmt.Sprint("[", i, "]"))
		}
